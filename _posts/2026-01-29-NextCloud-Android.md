---
layout: post
ID: 32
title: install nextcloud on android
author: Christian Gräfe
date: 2026-01-29 20:04:55
categories: Feuerwehr NextCloud
---

# NextCloud auf Android installieren

Diese Anleitung beschreibt die Installation und Konfiguration von **NextCloud** auf einem Android-Gerät inklusive App-Installation, Server-Konfiguration und Synchronisation.

---

## Voraussetzungen

- Android Gerät mit installiertem Google Play Store
- Zugangsdaten für das BKS-Portal
- NextCloud Server URL: https://cloud.bks-portal.rlp.de
---

## 1. Installation von NextCloud App

- Google Play Store öffnen
- "NextCloud" suchen und installieren
- App starten

## 2. Konfiguration der NextCloud App

### App öffen
![App öffnen](images/BKS-Cloud Schritt 1 - Öffnen.jpg)

### Server-URL eingeben

- Server-URL: `https://cloud.bks-portal.rlp.de` eingeben

![Server-URL eingeben](images/BKS-Cloud Schritt 2 - BKS-Portal.jpg)

### Anmelden

![Anmelden](images/BKS-Cloud Schritt 3 - Anmeldedaten.jpg)

### Kontozugriff erlauben

![Kontozugriff erlauben](images/BKS-Cloud Schritt 4 - Kontozugriff.jpg)

### Abschluss der Einrichtung

![Abschluss der Einrichtung](images/BKS-Cloud Schritt 5- Kontoabschluß.jpg)

### Öffnen der NextCloud App

![Öffnen der NextCloud App](images/BKS-Cloud Schritt 6 - Öffnen NextCloud.jpg)

### Zugriff auf Dateien gewähren

![Zugriff auf Dateien gewähren](images/BKS-Cloud Schritt 7 - Zugriff gewähren.jpg)

### Abschluß der Einrichtung

![Abschluß der Einrichtung](images/BKS-Cloud Schritt 8 - Ordnerübersicht.jpg)
